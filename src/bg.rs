...
impl BGMap {
    ...
    pub fn set_tile_id(&mut self, col: i16, row: i16, tile_id: impl Into<TileID>) {
        if let Some(index) = self.get_index(col, row) {
            self.tiles[index] = tile_id.into();
        }
    }

    pub fn set_flags(&mut self, col: i16, row: i16, flags: impl Into<TileFlags>) {
        if let Some(index) = self.get_index(col, row) {
            self.flags[index] = flags.into();
        }
    }

    pub fn get_tile_id(&self, col: i16, row: i16) -> Option<TileID> {
        let index = self.get_index(col, row)?;
        Some(self.tiles[index])
    }

    pub fn get_flags(&self, col: i16, row: i16) -> Option<TileFlags> {
        let index = self.get_index(col, row)?;
