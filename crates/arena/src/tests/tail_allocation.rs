...
fn test_tail_alloc_basic() {
    ...
    let source_slice = arena.alloc_slice_from_fn(10u32, |i| (i + 65) as u8).unwrap(); // "ABCDEFGHIJ"
...
fn test_tail_alloc_partial_copy() {
    ...
    let source_slice = arena.alloc_slice_from_fn(20u32, |i| (i + 48) as u8).unwrap(); // "0123456789..."
...
fn test_tail_alloc_with_existing_allocations() {
    ...
    let _existing1 = arena.alloc_slice_from_fn(100u32, |i| i as u8).unwrap();
    let _existing2 = arena.alloc_slice_from_fn(50u32, |i| (i + 100) as u8).unwrap();
    
    // Create source data
    let source_slice = arena.alloc_slice_from_fn(30u32, |i| (i + 200) as u8).unwrap();
...
fn test_tail_alloc_insufficient_space() {
    ...
    let _filler = arena.alloc_slice_from_fn(50u32, |i| i as u8).unwrap();
    
    // Create a small source slice
    let source_slice = arena.alloc_slice_from_fn(8u32, |i| (i + 100) as u8).unwrap();
...
fn test_tail_alloc_exactly_fits() {
    ...
    let _source = arena.alloc_slice_from_fn(source_size, |i| (i + 65) as u8).unwrap();
    ...
    if fill_amount > 0 {
        ...
        let _filler = arena.alloc_slice_from_fn(fill_size, |i| i as u8).unwrap();
    }
...
fn test_tail_alloc_zero_length() {
    ...
    let source_slice = arena.alloc_slice_from_fn(10u32, |i| (i + 100) as u8).unwrap();
...
fn test_tail_alloc_clears_tail_on_success() {
    ...
    let source_slice = arena.alloc_slice_from_fn(20u32, |i| i as u8).unwrap();
    ...
    let _big_alloc = arena.alloc_slice_from_fn(big_alloc_size, |i| i as u8);
...
fn test_tail_alloc_different_types() {
    ...
    let source_u32 = arena.alloc_slice_from_fn(5u32, |i| (i * 100) as u32).unwrap();
    ...
    let source_u16 = arena.alloc_slice_from_fn(3u32, |i| (i + 1000) as u16).unwrap();
...
fn test_tail_alloc_arena_clear_resets_tail() {
    ...
    // Do some allocations
    let _data = arena.alloc_slice_from_fn(100u32, |i| i as u8).unwrap();
    
    // Clear arena
    arena.clear();
    
    // After clear, should be able to use full capacity again including tail allocation
    let large_source = arena.alloc_slice_from_fn(400u32, |i| i as u8).unwrap();
...
fn test_tail_alloc_stress_multiple_operations() {
    ...
    for i in 0..10 {
        let size = (i + 1) * 10;
        let source = arena.alloc_slice_from_fn(size as u32, |j| (j + i * 50) as u8).unwrap();
