...
fn test_basic_slice() {
    ...
    let slice = arena.alloc_slice(&[0u32; 5]).unwrap();
    assert_eq!(slice.len(), 5);
    assert!(!slice.is_empty());

    let slice_data = arena.get_slice(slice).unwrap();
...
fn test_slice_with_function() {
    ...
    assert_eq!(slice_data, &[0, 10, 20, 30, 40]);
}
...
fn test_empty_slice() {
    ...
    let slice = arena.alloc_slice(&[0u32; 0]).unwrap();
    assert_eq!(slice.len(), 0);
    assert!(slice.is_empty());

    let slice_data = arena.get_slice(slice).unwrap();
    assert_eq!(slice_data.len(), 0);
}
...
fn test_slice_mutable_access() {
    ...
    {
        let slice_data = arena.get_slice_mut(slice).unwrap();
        slice_data[1] = 100;
    }

    let slice_data = arena.get_slice(slice).unwrap();
    assert_eq!(slice_data, &[0, 100, 2]);
}
...
fn test_slice_generational_safety() {
    ...
    let slice = arena.alloc_slice(&[0u32; 3]).unwrap();
...
fn test_slice_capacity_info() {
    ...
    let slice = arena.alloc_slice(&[0u64; 10]).unwrap();
...
fn test_multiple_slices() {
    ...
    let slice1_data = arena.get_slice(slice1).unwrap();
    let slice2_data = arena.get_slice(slice2).unwrap();

    assert_eq!(slice1_data, &[0, 1, 2]);
    assert_eq!(slice2_data, &[10, 11]);
}
...
fn test_slice_restore_safety() {
    ...
    let slice1 = arena.alloc_slice(&[0u32; 3]).unwrap();
    let checkpoint = arena.used();
    let slice2 = arena.alloc_slice(&[0u32; 2]).unwrap();
