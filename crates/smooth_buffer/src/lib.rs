...
impl<const CAP: usize, T: Float> SmoothBuffer<CAP, T> {
    ...
    pub fn gaussian_filter(&self) -> T {
        ...
        let sigma = T::from_usize_checked(self.filled_len).unwrap() / T::four();
        let center = T::from_usize_checked(self.filled_len - 1).unwrap() / T::two();
        ...
        for i in 0..self.filled_len {
            let distance = T::from_usize_checked(i).unwrap() - center;
