...
fn window_conf() -> Conf {
    tato_window_conf("Tato Macroquad Example", 900, 540)
}
...
async fn main() -> TatoResult<()> {
    ...
    tato.draw_text(
        &mut bg_map,
        "Animated palette - Macroquad Backend",
        TextOp {
            font: &FONT_LONG_MAP,
            tileset: ts_font,
            col,
    ...
    loop {
        ...
        // Handle close event in macroquad way
        if is_key_pressed(KeyCode::Escape) || is_key_down(KeyCode::LeftSuper) && is_key_pressed(KeyCode::Q) {
            break;
        }
